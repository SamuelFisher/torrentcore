...
class HttpTracker : ITracker
{
    ...
    /// <param name="httpClient">Http client used for making requests to the tracker.</param>
    public HttpTracker(
        ILogger<HttpTracker> logger,
        LocalTcpConnectionOptions tcpConnectionDetails,
        Uri baseUrl,
    ...
    private AnnounceResult ProcessResponse(Stream response)
    {
        ...
        if (dict["peers"] is BList peers)
        {
            foreach (BDictionary peer in peers)
            {
                string ip = ((BString)peer["ip"]).ToString();
                int port = (int)((BNumber)peer["port"]).Value;
        ...
        {
            // Compact response
            var peersString = (BString)dict["peers"];
